...
void applySift(QString Image1 ,QString Image2);
void siftFeatures(QString Image1);
ScaleSpace generate_gaussian_pyramid(const Image& img, float sigma_min=SIGMA_MIN,
                                            int num_octaves=N_OCT, int scales_per_octave=N_SPO);

ScaleSpace generate_dog_pyramid(const ScaleSpace& img_pyramid);

std::vector<Keypoint> find_keypoints(const ScaleSpace& dog_pyramid,
                                     float contrast_thresh=C_DOG, float edge_thresh=C_EDGE);

ScaleSpace generate_gradient_pyramid(const ScaleSpace& pyramid);

std::vector<float> find_keypoint_orientations(Keypoint& kp, const ScaleSpace& grad_pyramid,
                                              float lambda_ori=LAMBDA_ORI, float lambda_desc=LAMBDA_DESC);

void compute_keypoint_descriptor(Keypoint& kp, float theta, const ScaleSpace& grad_pyramid,
