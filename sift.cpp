...
ScaleSpace generate_gaussian_pyramid(const Image& img, float sigma_min,
...
ScaleSpace generate_dog_pyramid(const ScaleSpace& img_pyramid)
{
    ScaleSpace dog_pyramid = {
...
std::vector<Keypoint> find_keypoints(const ScaleSpace& dog_pyramid, float contrast_thresh,
                                     float edge_thresh)
{
...
ScaleSpace generate_gradient_pyramid(const ScaleSpace& pyramid)
{
    ScaleSpace grad_pyramid = {
        pyramid.num_octaves,
                                                     ...
                                                     float lambda_ori, float lambda_desc)
{
    ...
    ScaleSpace gaussian_pyramid = generate_gaussian_pyramid(input, sigma_min, num_octaves,
                                                                   scales_per_octave);
    ScaleSpace dog_pyramid = generate_dog_pyramid(gaussian_pyramid);
    std::vector<Keypoint> tmp_kps = find_keypoints(dog_pyramid, contrast_thresh, edge_thresh);
    ScaleSpace grad_pyramid = generate_gradient_pyramid(gaussian_pyramid);
...
float SSD(std::array<uint8_t, 128>& a, std::array<uint8_t, 128>& b)
{
    float dist = 0;
    for (int i = 0; i < 128; i++) {
        int di = (int)a[i] - b[i];
...
float normalized_cross_correlation(std::array<uint8_t, 128>& a, std::array<uint8_t, 128>& b)
{
    ...
    for (int i = 0; i < 128; i++) {
        ...
        den_b += (b[i] - mean_b) * (b[i] - mean_b);
    }
    float den = std::sqrt(den_a * den_b);
    if (den == 0) {
        return 0;
    }
    return num / den;
                                                       ...
                                                       float thresh_absolute)
{
    ...
    for (int i = 0; i < a.size(); i++) {
        ...
        for (int j = 0; j < b.size(); j++) {
            ...
            if (num==1){
            dist = SSD(a[i].descriptor, b[j].descriptor);}
            else if(num==2){
            dist = normalized_cross_correlation(a[i].descriptor, b[j].descriptor);
            }
...
void sift::applySift(QString Image1,QString Image2){
    Image img(Image1.toStdString());
    Image img2(Image2.toStdString());
...
//    std::vector<sift::Keypoint> kps1_ssd = sift::find_keypoints_and_descriptors(img);
//    std::vector<sift::Keypoint> kps2_ssd = sift::find_keypoints_and_descriptors(img2);
    std::vector<std::pair<int, int>> matches_SSD = sift::find_keypoint_matches(kps1, kps2,1);
    std::vector<std::pair<int, int>> matches_NCC = sift::find_keypoint_matches(kps1, kps2,2);
    Image matchesImageSSD = sift::draw_matches(img, img2, kps1, kps2, matches_SSD);
    Image matchesImageNCC = sift::draw_matches(img, img2, kps1, kps2, matches_NCC);
    matchesImageSSD.save("../a03-team-21-1/ssd.png");
    matchesImageNCC.save("../a03-team-21-1/ncc.png");
}

void sift::siftFeatures(QString Image1 ){
    Image img(Image1.toStdString());
    img = rgb_to_grayscale(img);
    std::vector<sift::Keypoint> kps_a = sift::find_keypoints_and_descriptors(img);
    Image result = sift::draw_keypoints(img,  kps_a);
    result.save("../a03-team-21-1/siftFeatures.png");
}
